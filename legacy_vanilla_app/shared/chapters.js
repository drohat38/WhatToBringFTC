 ...
 *
 * TO ADD A NEW CITY:
 *   1. Add one object to the CHAPTERS array below.
 *   2. That's it. All three products use this file automatically.
 *
 * FIELDS:
 *   slug  — URL-safe identifier used as ?chapter= param (lowercase, hyphens)
 *   name  — Display name shown in the UI
 *   page  — Wix event page path (relative to tangocharities.org)
 *   week  — Which Saturday of the month: 1, 2, 3, or 4
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
/**
 * getChapter(slug) → chapter object or null
 * Used by all products to resolve ?chapter= URL param.
 */
function getChapter(slug) {
  if (!slug) return null;
  return CHAPTERS.find(function(c) { return c.slug === slug; }) || null;
}

// Node/CommonJS export (ignored in browser)
if (typeof module !== 'undefined') module.exports = { CHAPTERS: CHAPTERS, getChapter: getChapter };
